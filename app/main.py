...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    ...
    if payload.get("type") != "access":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Tipo de token inválido. Use um access token.", headers={"WWW-Authenticate": "Bearer"})
    return payload
...
@app.get("/")
def root():
    return {"message": "API online", "docs": "/docs", "openapi": "/openapi.json"}
...
def refresh_token_endpoint(refresh_data: RefreshRequest):
    try:
        ...
        if payload.get("type") != "refresh":
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Tipo de token inválido. Use um refresh token.", headers={"WWW-Authenticate": "Bearer"})
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro ao renovar token")

@app.get("/api/v1/auth/verify", tags=["Autenticação"])
def verify_token(current_user: dict = Depends(get_current_user)):
    return {"message": "Token válido", "username": current_user.get("username"), "expires_at": current_user.get("exp")}
...
def trigger_scraping(current_user: dict = Depends(admin_required)):
    ...
    scrape_books()
    return {"message": "Scraping iniciado com sucesso", "triggered_by": current_user.get("username"), "timestamp": datetime.datetime.utcnow().isoformat()}

@app.get("/api/v1/books/search", tags=["Obrigatório"])
def search_books_endpoint(title: Optional[str] = Query(None, description="Título do livro para busca parcial"), category: Optional[str] = Query(None, description="Categoria do livro para busca parcial"), db: Session = Depends(get_db)):
    try:
        if not title and not category:
            raise HTTPException(status_code=400, detail="Pelo menos um parâmetro de busca deve ser fornecido (title ou category)")
        ...
        if not books:
            return {"message": f"Nenhum livro encontrado com os critérios: {filter_message}", "data": [], "total": 0, "filters": {"title": title, "category": category}}
        return {"message": f"{len(books)} livro(s) encontrado(s) com os critérios: {filter_message}", "data": [{"id": book.id, "titulo": book.titulo, "preco": float(book.preco) if book.preco is not None else 0.0, "rating": float(book.rating) if book.rating is not None else 0.0, "categoria": getattr(book, "categoria", None)} for book in books], "total": len(books), "filters": {"title": title, "category": category}}
    except HTTPException:
        raise
    except Exception as e:
...
def get_top_rated_books_endpoint(db: Session = Depends(get_db)):
    try:
        ...
        return {"message": "Livros com melhor avaliação", "data": [{"id": book.id, "titulo": book.titulo, "preco": float(book.preco) if book.preco is not None else 0.0, "rating": float(book.rating) if book.rating is not None else 0.0, "categoria": getattr(book, "categoria", None)} for book in books]}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro interno: {str(e)}")

@app.get("/api/v1/books/price-range", tags=["Opcionais"])
def get_books_by_price_range(min_price: float = Query(None, alias="min", description="Preço mínimo"), max_price: float = Query(None, alias="max", description="Preço máximo"), db: Session = Depends(get_db)):
    try:
        if min_price is None and max_price is None:
            raise HTTPException(status_code=400, detail="Pelo menos um parâmetro deve ser fornecido (min ou max)")
        ...
        if not books:
            return {"message": "Nenhum livro encontrado na faixa de preço especificada", "data": [], "total": 0, "filters": {"min_price": min_price, "max_price": max_price}}
        return {"message": f"{len(books)} livro(s) encontrado(s) na faixa de preço", "data": [{"id": book.id, "titulo": book.titulo, "preco": float(book.preco) if book.preco is not None else 0.0, "rating": float(book.rating) if book.rating is not None else 0.0, "categoria": getattr(book, "categoria", None)} for book in books], "total": len(books), "filters": {"min_price": min_price, "max_price": max_price}}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro interno: {str(e)}")
